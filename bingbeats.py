...
response = requests.get('https://hunter.io/trial/v2/domain-search?offset=0&domain=mymakeupbrushset.com&format=json').json()
urls_to_visit = response['data']['emails'][0]['sources'][0]['uri']
get_emails = requests.get(urls_to_visit).text
soup = BeautifulSoup(get_emails)
emails = re.findall(r"[a-z0-9\.\-+_]+@[a-z0-9\.\-+_]+\.[a-z]+", soup, re.I)
print emails
